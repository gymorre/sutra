...
async function generateTextSticker(text) {
  ...
  const PADDING = 48;          // padding/jarak tepi kotak yang cukup
  const MAX_WIDTH = SIZE - PADDING * 2;
  const LINE_HEIGHT_RATIO = 1.15; // Jarak baris standar yang pas untuk font regular
  ...
  // Konversi teks ke lowercase (huruf kecil semua) dan bersihkan whitespace
  ...
  function wrapText(txt, fSize) {
    ctx.font = `500 ${fSize}px Arial, Helvetica, "Nimbus Sans"`;
  ...
  const startY = (SIZE - totalTextHeight) / 2; // Memosisikan teks di tengah secara vertikal

  ctx.font = `500 ${fontSize}px Arial, Helvetica, "Nimbus Sans"`;
  ctx.fillStyle = "#000000";
  ctx.textBaseline = "top";
  ctx.textAlign = "left";

  // Gambar setiap baris rata kiri (left-aligned) dengan warna hitam pekat (#000000)
...
/**
 * Convert PNG buffer ke buffer stiker menggunakan Sharp (WebP format) bersih tanpa efek/blur
 */
