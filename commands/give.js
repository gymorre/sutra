// commands/give.js
import { addBalance, getUser, getUserByNickname } from "../utils/economy.js";
...
export async function execute({ sender, args, reply, msg }) {
  ...
  if (!amountArg) {
    return reply(
      `${config.ui.line}\n┃ 🔧 ADMIN GIVE\n${config.ui.line}\n\n` +
      `Format: *!give <id/phone/nickname/@tag> <jumlah>*\n` +
      `Atau balas pesan target: *!give <jumlah>*\n\n` +
      `Contoh:\n` +
  ...
  return reply(
    `${config.ui.line}\n┃ 🔧 ADMIN GIVE\n${config.ui.line}\n\n` +
    `✅ Berhasil menambahkan ${config.currencySymbol}${amount.toLocaleString()}\n\n` +
    `👤 Player: ${targetUser.nickname} (@${playerNum})\n` +
    `💰 Balance baru: ${config.currencySymbol}${newBalance.toLocaleString()}\n\n${config.ui.line}`
  );
}
